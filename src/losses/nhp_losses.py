...
class NHPLoss(nn.Module):
    ...
    def make_dtime_loss_samples(
        time_delta_seq: torch.Tensor, loss_integral_num_sample_per_step: int
    ) -> torch.Tensor:
    ...
    def compute_loss(
        ...
        interval_t_sample = self.make_dtime_loss_samples(
            time_delta_seqs[:, 1:], self.loss_integral_num_sample_per_step
        )

        # [batch_size, num_times = max_len - 1, num_mc_sample, hidden_size]
        state_t_sample = self.compute_states_at_sample_times(
            seq_encoder, decay_states, interval_t_sample
        )
...
class AttnNHPLoss(NHPLoss):
    ...
    def compute_intensities_at_sample_times(
        ...
        if sample_times.size()[1] < time_seqs.size()[1]:
            ...
            sample_times = time_seqs[:, :, None] + torch.tile(
                sample_times, [1, time_seqs.size()[1], 1]
            )
        ...
        if compute_last_step_only:
            lambdas = seq_encoder.layer_intensity(encoder_output[:, -1:, :, :])  # type: ignore
        else:
            # [batch_size, seq_len, num_samples, num_event_types]
            lambdas = seq_encoder.layer_intensity(encoder_output)  # type: ignore
        return lambdas
    ...
    def compute_loss(
        ...
        lambda_at_event = seq_encoder.layer_intensity(enc_out)  # type: ignore

        # 2. compute non-event-loglik (using MC sampling to compute integral)
        # 2.1 sample times
        # [batch_size, seq_len, num_sample]
        temp_time = self.make_dtime_loss_samples(
            time_delta_seqs[:, 1:], self.loss_integral_num_sample_per_step
        )
