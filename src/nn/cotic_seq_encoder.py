"""The file with the COTIC encoder."""
...
class CoticEncoder(AbsSeqEncoder):
    ...
    def forward(
        ...
        Args:
        ----
            event_times (torch.Tensor): timestamps (extracted from PaddedBatch)
            event_types (torch.Tensor): event type features (extracted from PaddedBatch) and passed through nn.Embedding
