...
def preprocess_data():
    ...
    df_target_train, df_target_valid = train_test_split(
        df_target_train,
        test_size=500,
        stratify=df_target_train["target_flag"],
        random_state=142,
    )
    print(
        "Split {} records to train: {}, valid: {}, test: {}".format(
            *[
                len(df_)
                for df_ in [df_target, df_target_train, df_target_valid, df_target_test]
            ]
        )
    )
    ...
    with wandb.init() as run:
        ...
        sup_data = PtlsDataModule(
            train_data=SeqToTargetDataset(
                dataset_train, target_col_name="target_flag", target_dtype=torch.long
            ),
            valid_data=SeqToTargetDataset(
                dataset_valid, target_col_name="target_flag", target_dtype=torch.long
            ),
            test_data=SeqToTargetDataset(
                dataset_test, target_col_name="target_flag", target_dtype=torch.long
            ),
